...
<head>
	<meta charset="utf-8">
	<title>総合実績データ<?php #echo $name; ?></title>
     ...
     <header>
      ...
      <div class="container">
        <div class="navbar-header">
          ...
          <a class="navbar-brand" href="../index">redirecttool</a>
        </div>
        <div class="collapse navbar-collapse">
          <ul class="nav navbar-nav">
            <li><a href="index">ホーム</a></li>
            <li class="active"><a href="read">総合実績データ／リスト</a></li>
            <!-- <li><a href="campaigndetail">個別実績データ</a></li> -->
		...
		<div class="row">
			<div class="col-md-12">
				<!--
                                ...
                                <h1>[<?php echo $medianame; ?>]URL別実績確認</h1>
                                <p class="lead">ここでは、URLの確認とクリック数・成果数の集計を表示します。</p>
				<hr>
                ...
                <table class="table table-striped table-bordered table-hover table-condensed table-responsive">
                    <thead>
                        <tr class="success">
                            <!-- 
                            <td class="col-xs-12 col-sm-5 col-md-6">キャンペーン名</td><td class="col-xs-12 col-sm-1 col-md-2">パラメータ</td>
                            -->
                            <td class="col-xs-12 col-sm-3 col-md-2">カウント数</td><td class="col-xs-12 col-sm-3 col-md-2">成果数</td></tr>
                    </thead>
                    <tbody>
                        <tr class="warning">
                            <!--
                            ...
                            ?></td><td><?php echo $pid; ?></td>
                            -->
                            <td><span style="font-size:24px;"><?php echo $clickcount; ?></span>回</td><td><span style="font-size:24px;"><?php echo $cvcount; ?></span>回</td></tr>
                    </tbody>
               </table>
            ...
            <form method="post" action="./read">
                <!-- <form method="post" action="main.php?mid=<?php #echo $mid; ?>"> -->
		<p>
                <!--
                <div class="form-group">
                        <label for="name">パラメータ[PID]</label>
		<input size="20" type="text" maxlength="20" name="pid" value="<?php $pid; ?>" class="form-control" required>
                </div>
                -->
                <div class="form-group">
                        <label for="name">計測期間で絞り込む</label>
		<!-- 
                <input type="hidden" name="pid" value="<?php #echo $pid; ?>" class="form-control" required>
                -->
                </div>
                
                <div class="form-group">
                    <label for="name">以下のプルダウンメニューで期間（年月日）を選択してください</label>
                    <div class="col-xs-12 form-inline">
                <select name="year" class="form-control">
                    <option value="____">選択</option>
                    <option value="2015">2015年</option>
                </select>
                      
                <select name="month" class="form-control">
                    <option value="__">選択</option>
                    <?php
                    for ($num = 01; $num < 13; $num++){
                      $numb = sprintf("%02d", $num);
                      print '<option value="'.$numb.'">'.$num.'月</option>';
                    }
                     ?> 
                </select>
                ...
                <select name="day" class="form-control">
                    <option value="__">選択</option>
                    <?php
                    for ($num = 01; $num < 32; $num++){
                      $numb = sprintf("%02d", $num);
                      print '<option value="'.$numb.'">'.$num.'日</option>';
                    }
                     ?> 
                </select>
                        </div> 
                ...
                <div class="form-group">
                    <div class="col-xs-12">
                     <input type="submit" value="reg" class="btn">
                    </div>
                </div>
               ...
               <h3>集計対象キャンペーンリスト</h3>
                <table class="table table-striped table-bordered table-hover table-condensed">
                    <thead>
                            <tr><td class="col-xs-12 col-sm-4 col-md-5">キャンペーン名</td><td class="col-xs-12 col-sm-4 col-md-5">URL</td><td class="col-xs-12 col-sm-4 col-md-2">-</td></tr>
                    </thead>
                    
                   
                    <tbody>
                        <?php foreach ($sql_result_alllist as $row_alllist) { ?>
                        <tr>
                            <td><?php print $row_alllist['campname'];?></td>
                            <td class="textheight">http://<?php print $hostname.$rd_dir; ?>/rd.php?mid=<?php print $row_alllist['medianame'];?>&pid=<?php print $row_alllist['parameterid'];?></td>
                            <td>
                                <form method="post" action="./campaigndetail">
                                    <input type="hidden" name="pid" value="<?php print $row_alllist['parameterid'];?>">
                                    <input type="hidden" name="mid" value="<?php print $row_alllist['medianame'];?>">
